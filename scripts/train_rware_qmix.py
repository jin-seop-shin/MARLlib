...
RESULTS_DIR = os.path.abspath(
    os.environ.get("RWARE_RESULTS_DIR", "runs/rware_mappo_gru_small_20m_tensorboard")
)
RWARE_GYM_ID = os.environ.get("RWARE_GYM_ID", "rware-small-4ag-v2")
STOP_TIMESTEPS = int(os.environ.get("RWARE_STOP_TIMESTEPS", "40000000"))
...
QMIX_BATCH_EPISODE = int(os.environ.get("RWARE_QMIX_BATCH_EPISODE", "8"))
QMIX_BUFFER_SIZE = int(os.environ.get("RWARE_QMIX_BUFFER_SIZE", "10000"))
QMIX_EPSILON_TIMESTEPS = int(os.environ.get("RWARE_QMIX_EPSILON_TIMESTEPS", "5000000"))
QMIX_TARGET_UPDATE_FREQ = int(os.environ.get("RWARE_QMIX_TARGET_UPDATE_FREQ", "100"))
QMIX_FINAL_EPSILON = float(os.environ.get("RWARE_QMIX_FINAL_EPSILON", "0.05"))
QMIX_LR = float(os.environ.get("RWARE_QMIX_LR", "0.0005"))
...
def latest_checkpoint():
    stage_dir = os.path.join(RESULTS_DIR, "qmix_gru_{}".format(RWARE_GYM_ID))
...
def main():
    ...
    qmix = marl.algos.qmix(
        hyperparam_source="common",
        batch_episode=QMIX_BATCH_EPISODE,
        buffer_size=QMIX_BUFFER_SIZE,
        epsilon_timesteps=QMIX_EPSILON_TIMESTEPS,
        target_network_update_freq=QMIX_TARGET_UPDATE_FREQ,
        final_epsilon=QMIX_FINAL_EPSILON,
        lr=QMIX_LR,
    )
    model = marl.build_model(env, qmix, {"core_arch": "gru", "encode_layer": "128-256"})
