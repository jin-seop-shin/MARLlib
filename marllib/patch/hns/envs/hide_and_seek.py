...
from marllib.patch.hns.wrappers.multi_agent import (SplitMultiAgentActions,JoinMultiAgentActions,
                                           SplitObservations, SelectKeysWrapper)
from marllib.patch.hns.wrappers.util import (DiscretizeActionWrapper, ConcatenateObsWrapper,
                                    MaskActionWrapper, SpoofEntityWrapper,
                                    DiscardMujocoExceptionEpisodes,
                                    AddConstantObservationsWrapper)
from marllib.patch.hns.wrappers.manipulation import (GrabObjWrapper, GrabClosestWrapper,
...
from marllib.patch.hns.wrappers.line_of_sight import (AgentAgentObsMask2D, AgentGeomObsMask2D,
                                             AgentSiteObsMask2D)
from marllib.patch.hns.wrappers.prep_phase import (PreparationPhase, NoActionsInPrepPhase,
                                          MaskPrepPhaseAction)
from marllib.patch.hns.wrappers.limit_mvmnt import RestrictAgentsRect
from marllib.patch.hns.wrappers.team import TeamMembership
from marllib.patch.hns.wrappers.food import FoodHealthWrapper, AlwaysEatWrapper
...
from marllib.patch.hns.modules.food import Food
from marllib.patch.hns.modules.world import FloorAttributes, WorldConstants
from marllib.patch.hns.modules.util import (uniform_placement, close_to_other_object_placement,
...
def quadrantOrset_placement(grid, obj_size, metadata, random_state, agent_id=None):
    '''
        Places object within the bottom right quadrant of the playing field
    '''
    
    if 'set_agent_state' in metadata and agent_id is not None:
...
def uniformOrset_placement(grid, obj_size, metadata, random_state, box_id=None):
    '''
        Places object within the bottom right quadrant of the playing field
    '''
    if 'set_box_state' in metadata:
...
def outside_wall_set_placement(grid, obj_size, metadata, random_state, ramp_id=None):
    '''
        Places object outside of the bottom right quadrant of the playing field
    '''
    if 'set_ramp_state' in metadata:
...
def outside_seeker_set_placement(grid, obj_size, metadata, random_state, seeker_id=None):
    '''
        Places object outside of the bottom right quadrant of the playing field
    '''
    if 'set_agent_state' in metadata:
        pos = np.array(metadata['set_agent_state'][seeker_id-metadata['uniform_num_hider']])
        return pos
    else:
        grid_size = len(grid)
        qsize = metadata['quadrant_size']
...
def HideAndSeekEnv(args, n_substeps=15, horizon=60, deterministic_mode=True,
             ...
             n_boxes=1, n_ramps=4, n_elongated_boxes=0,
             ...
             prep_fraction=0.0, prep_obs=True,
             team_size_obs=False,
    ...
    if scenario == 'randomwalls':
        ...
        box_placement_fn = quadrant_placement # rejection_placement
        # ramp_placement_fn = nearwall_placement 
        ramp_placement_fn = outside_wall_set_placement # rejection_placement_ramp
        hider_placement = uniform_placement if quadrant_game_hider_uniform_placement else quadrantOrset_placement # rejection_placement
        agent_placement_fn = [hider_placement] * n_hiders + [outside_seeker_set_placement] * n_seekers # rejection_placement_agent
    else:
